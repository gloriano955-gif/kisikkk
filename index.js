...
const DEFAULT_PROSE_TEMPLATE = `다음 내용을 자연스럽고 간결한 한 문단의 줄글로 다듬어줘. 소설체가 아닌 일반 문장으로 써. 원래 의미는 유지하되 어색한 표현만 정리해줘:\n[대필]`;
...
const defaultSettings = {
    ...
    draftInput: '',
    // Generation mode toggle: false = 소설 모드 (/impersonate), true = 줄글 모드 (/genraw)
    proseMode: false,
};
...
function buildInstructionPrompt(userInput) {
    ...
    let template;
    if (settings.proseMode) {
        // 줄글 모드: 사용자 템플릿이 기본값(소설)이면 줄글 기본값으로 대체, 아니면 사용자 커스텀 템플릿 사용
        const isDefaultTemplate = settings.instructionTemplate === DEFAULT_INSTRUCTION_TEMPLATE;
        template = isDefaultTemplate ? DEFAULT_PROSE_TEMPLATE : (settings.instructionTemplate || DEFAULT_PROSE_TEMPLATE);
    } else {
        template = settings.instructionTemplate || DEFAULT_INSTRUCTION_TEMPLATE;
    }
...
async function runImpersonate(userInput) {
    ...
    try {
        if (settings.proseMode) {
            // 줄글 모드: /genraw로 직접 생성 (캐릭터/스토리 컨텍스트 없이 순수 지시만 전달)
            const commands = `/genraw ${instruction}`;
            const result = await ctx.executeSlashCommandsWithOptions(commands, {
                handleParserErrors: true,
                handleExecutionErrors: false,
            });
            generated = result?.pipe || '';
...
/impersonate await=true`;
            ...
            const textarea = document.querySelector('#send_textarea');
            generated = textarea?.value || result?.pipe || '';
        }
...
function createGhostwriteUI() {
    ...
    if (!panel) {
                ...
                <div class="gw-input-row">
                    ...
                    <div class="gw-input-actions">
                        <div class="gw-util-btns">
                            ...
                            <button id="gw-mode-btn" class="gw-icon-btn" title="${settings.proseMode ? '줄글 모드' : '소설 모드'}">
                                <i class="fa-solid ${settings.proseMode ? 'fa-align-left' : 'fa-book-open'}"></i>
                            </button>
                        </div>
        ...
        panel.querySelector('#gw-mode-btn')?.addEventListener('click', () => {
            const s = getSettings();
            s.proseMode = !s.proseMode;
            const btn = panel.querySelector('#gw-mode-btn');
            if (btn) {
                btn.title = s.proseMode ? '줄글 모드' : '소설 모드';
                btn.innerHTML = `<i class="fa-solid ${s.proseMode ? 'fa-align-left' : 'fa-book-open'}"></i>`;
            }
            saveSettingsDebounced();
            toastr.info(s.proseMode ? '줄글 모드로 전환' : '소설 모드로 전환');
        });
